...
array_lengths.push( findLongestWord( ["Ouagadougou", "Bujumbura", "Brazzaville", "Lagos", "Cairo"] ) );
...
function beautifyNumbers( array )
{
	...
	for(var i = 0; i < length; i++){
		if(isFinite(array[i])){
			var beautyNumber = array[i];
			while(beautyNumber.toString().length > 1){
				var digitsArray = beautyNumber.toString().split('');
				var digitsArrayLength = digitsArray.length;
				beautyNumber = 0;
				for(var j = 0; j < digitsArrayLength; j++){
					beautyNumber += parseInt(digitsArray[j]);
				}
			}
			array[i] = beautyNumber;
		}
	}
